 ...
 * guest's RAM image. In 64bit it looks for a given signature in a 
 * contiguous memory page and patches the signature in-memory allowing
...
void DoInjectionRoutine(void *,char *,size_t,size_t);

DWORD64 FindSignature(DWORD64, size_t , char *, size_t ,char *);
...
typedef struct signatures {
	...
	//signature, signature mask, byte size of signature, patching offset (number of bytes where to write the patch), patch to apply, byte size of patch
SIG list_sig[] = { 
...
BOOL APIENTRY DllMain(HMODULE hModule,DWORD ul_reason_for_call,LPVOID lpReserved)
{
	...
	case DLL_PROCESS_ATTACH:
		if(AllocConsole()) {
			...
			SetConsoleTitle("VMInjector x64 DLL v0.1 (c) 2012 by Marco Batista");
...
DWORD64 MemoryMapperThread() {
	DWORD64 ddMemoryAddress;
	DWORD64 ddMemoryLeft;
	DWORD64 ptrPossible;
	HMODULE hPSAPI;
	...
	ptrPossible=FindSignature(ddMemoryAddress,ddMemoryLeft,list_sig[i].sig,list_sig[i].sig_length,list_sig[i].sig_mask);
	if(ptrPossible!=NULL){
		printf(" <-- Found Signature Match\n");
		DoInjectionRoutine((void *)ptrPossible,list_sig[i].patch,list_sig[i].patchoffset,list_sig[i].patch_length);			
	}else 
	...
	system("pause");
	FreeConsole();
	FreeLibraryAndExitThread(GetModuleHandle("vminjector64.dll"),NULL);
	return true;
}

void DoInjectionRoutine(void *pvAPointer,char *patch,size_t patchoffset,size_t patch_length) {
	void *pBasePtr;
	
	pBasePtr = (char *)pvAPointer+patchoffset;
	memcpy(pBasePtr,patch,patch_length);
	return;
}


DWORD64 FindSignature(DWORD64 base_addr,size_t base_len, char *sig_str, size_t sig_len,char *sig_mask) {
    DWORD64 pBasePtr;
    DWORD64 pEndPtr; 
    size_t i;

	pBasePtr = base_addr;
	pEndPtr = (DWORD64)(((char *)base_addr+base_len));
	...
	while(pBasePtr < pEndPtr) { 
		...
		if(((char *)pBasePtr)[0] != sig_str[0]) { //there is function memchar that might work better....?
			pBasePtr++;
			continue;
		}
		if(((char *)pBasePtr)[sig_len-1] != sig_str[sig_len-1]) {
			pBasePtr++;
			continue;
		}
		for(i = 0;i < sig_len;i++) {
			if(sig_mask[i] == 0x3F) //if signature mask is "?", skip it.
				continue;
			else{
				if(sig_str[i] != ((char *)pBasePtr)[i])
					break;
			}
		} 
...
DWORD64 GetMemorySizeLeft(void *pvAPointer) {
	...
	if(VirtualQuery(pvAPointer,&anMBI,sizeof(anMBI))<sizeof(anMBI)) 
		return(0);
	pvMemoryEnd=(char *)anMBI.BaseAddress+anMBI.RegionSize;
	ddSizeLeft=(DWORD64)(((char *)pvMemoryEnd-(char *)pvAPointer));
	return(ddSizeLeft);
}

DWORD64 GetNextMemoryAddress(DWORD64 ddCurrentAddress) { 
	DWORD64 ddMemoryAddress;
	DWORD64 ddAddressIncrement;
